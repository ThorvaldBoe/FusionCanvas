...
| Store Editor focused headless coverage | Pass | `StoreEditorHeadlessTests` passes 8/8, including Catalog & mockups navigation, strategy helper text, progressive sections, offering-context synchronization, explicit missing-normalized-record state, future template state, and layout margins after filtering non-laid-out hidden controls. |
...
| Blueprint Offering detail uses one authoritative context | Pass | `CatalogSetupViewModelTests.RequestedOfferingIdentityIsAuthoritativeAndNeverFallsBack` and the offering-detail headless tests verify stable-ID synchronization, absence of a second offering selector, hidden normalized forms plus one clear unavailable message when no matching record exists, and one consolidated Advanced section. Focused combined run passed 10/10; complete App suite passed 489/489. |
