...
The completed offering editor removes that temporary mixed presentation. `CatalogSetupViewModel` owns all offering-detail sections and mutates only `BlueprintOffering`, typed Options/Values, explicit `OfferingVariant`, `OfferingPlaceholder`, and Mockup Template application services. The older Product/offering graph remains a transitional compatibility projection for existing consumers outside this editor, not a second editable UI model. A focused application compatibility synchronizer repairs schema-11 databases written by earlier builds that contain compatibility records without same-ID normalized records, then projects confirmed normalized Basics, Variants, and Placeholders back to compatibility records as needed. Repair is additive and idempotent: it preserves equivalent Blueprint, offering, Variant, and Placeholder IDs, creates only missing provider/Option/Value identities, never overwrites existing normalized records, and never fabricates templates.

Options precede Variants because concrete Variants select stable Option Value identities. Placeholder creation selects compatible normalized Variants. Each collection shows records and an explicit Add action first; creation fields are hidden until that action is activated. This removes the always-open raw form and free-text Color/Size path that made ownership and prerequisites unclear.
...
- Add an idempotent application compatibility synchronizer used when Store catalog state loads and after normalized mutations: repair missing same-ID normalized records from compatibility data, then mirror authoritative normalized Basics/Variants/Placeholders for remaining compatibility readers without exposing legacy editors.
- Move all Blueprint Offering detail presentation into `CatalogSetupViewModel`: normalized editable Basics, progressively disclosed Options/Values, explicit Variant selection by Option Value identity, Placeholder compatibility by Variant identity, Mockup Templates, and one Advanced section in dependency order.
- Remove Store Editor offering-detail bindings and actions for legacy free-text Color/Size Variants and design areas; retain the older service only for the still-transitional Blueprint overview and creation entry points until those are replaced separately.
