...
FusionCanvas SHALL retain a secondary Edit Properties surface for notes/context, appearance metadata when available, accessible destination selection, archive, and restore.

#### Scenario: User opens detailed properties
- **WHEN** the user invokes Edit Properties for an active group
- **THEN** FusionCanvas opens a focused surface with that group preselected
...
#### Scenario: User leaves meaningful unsaved property changes
- **WHEN** the user closes or switches the detailed editor with meaningful unsaved changes
- **THEN** FusionCanvas offers save, discard, or cancel
- **AND** choosing cancel retains the draft and choosing discard leaves persisted data unchanged
