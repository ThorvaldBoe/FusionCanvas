...
### D4: Avalonia surface adds a narrow instruction field per corner beside its value box

The per-corner action `Grid` keeps one row per corner (3 rows x 4 columns: `Auto,*,Auto,Auto` = label, value box, Fine tune, Change). Each instruction field is a small, narrow single-line `TextBox` placed in the same row in columns 2-3 (spanning the two button columns) with `VerticalAlignment="Bottom"`, so it tucks under the corner's Fine tune/Change buttons while sitting just to the right of (beside) that corner's value box. It uses `PlaceholderText="Instructions"`, no visible label, a distinct accessible name (e.g. `Instructions for Concept idea`), and `IsReadOnly` bound to `!ItemInspector.CanEditStage`. The field is two-way bound to its corner's instruction property with `UpdateSourceTrigger=PropertyChanged`.

Alternative considered: a full-width instruction row spanning the value box and buttons. Rejected after review because it made the panel significantly taller; the intended placement is a narrow field tucked under the buttons beside the value box, so the grid stays compact at one row per corner.
...
3. **Avalonia surface** — In `src/FusionCanvas.App/Views/MainWindow.axaml`, add one narrow single-line `TextBox` per corner within each corner's existing action-row grid cell, placed in columns 2-3 with `VerticalAlignment="Bottom"` so it sits under the Fine tune/Change buttons beside the corner's value box, styled with `PlaceholderText="Instructions"`, two-way binding to the corner instruction property, a distinct accessible name, and read-only bound to `!ItemInspector.CanEditStage`.
