...
| Instruction fields sit under the buttons beside the value box | Avalonia headless view test | Pass | `InstructionFields_PresentBelowButtonPairs_WithPlaceholderAndTwoWay` asserts each field is below its corner's Change button, to the right of the value box, narrower than the value box, placeholder `Instructions`, and two-way bound. | Headless control-property verification does not make a pixel-perfect visual assertion. |
