...
| Settings keyboard access, primary editing, artwork target/defaulting, explicit override, target reset, transparency default, busy/cancel/disabled/read-only states | `AiSettingsViewTests`; Design Area editor headless cases; `DesignStageToolHeadlessTests.ArtworkGenerationSection_ExposesTargetAndTransparencyControls`; `SllSectionHeadlessTests` | Pass |
