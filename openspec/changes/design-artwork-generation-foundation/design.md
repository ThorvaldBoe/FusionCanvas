...
The Design selector contains all active areas from the selected offering. A non-primary user selection lasts only for the open Item session. Changing Listing Configuration cancels current work and resets the selector to the new offering's primary or no target.
...
| `Primary Design Area supplies the initial target`; `Offering has no primary Design Area`; `No valid Listing Configuration target exists`; `Listing Configuration changes`; `Explicit target is session scoped` | Design view-model tests and headless selector/change tests. |
