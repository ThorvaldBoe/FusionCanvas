...
The FusionCanvas production layers SHALL group production types into cohesive capability folders and namespaces (for example `Items`, `Stores`, `Niches`, `Groups`, `Tags`, `Assets`, `Prompts`, `Workflow`, `Navigation` in Domain, and the matching capability folders in Application and Integration), with a narrow shared root for cross-cutting primitives where justified, rather than a single catch-all folder.
