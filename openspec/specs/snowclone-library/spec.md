...
Defines the application-wide snowclone library: structured reusable phrase templates, their management dialog, CSV interchange, and the opt-in bundled starter library behavior.
...
FusionCanvas SHALL ship a UTF-8 CSV starter library using the same `Phrase,Guidance` contract as user interchange and SHALL add the bundled curated snowclones to the local library only when the creator explicitly imports them, without silently overwriting or resurrecting later user changes.

#### Scenario: Snowclone library stays empty by default
