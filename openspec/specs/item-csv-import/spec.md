...
Defines accepted behavior for importing Design items from a semi-colon-delimited CSV-like text file into the navigation tree, including the standard text format, context-menu entry points, destination targeting, column-to-field mapping with stage selection, row validation, and the import dialog with raw source, preview, and syntax check.
