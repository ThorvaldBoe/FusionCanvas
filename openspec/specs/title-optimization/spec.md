...
Defines accepted behavior for the Optimize command next to the Working title in the listing inspector Overview: deriving availability from a Title AI purpose and item content/editability, generating short titles with a bounded uniqueness loop and numeric-suffix fallback, and commiting accepted titles through the automatic-save path.
