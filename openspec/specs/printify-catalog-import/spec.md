...
The application SHALL retrieve Printify catalog data only for a saved Store using the Store's Shopify + Printify strategy, configured credential, and selected Printify shop context, and SHALL not mutate local catalog data during retrieval or preview.

#### Scenario: Saved Printify Store opens catalog import
- **WHEN** the user opens Catalog & mockups for a saved Store with Shopify + Printify strategy and an available credential
