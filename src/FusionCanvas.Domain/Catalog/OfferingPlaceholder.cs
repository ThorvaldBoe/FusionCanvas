...
public sealed record OfferingPlaceholder
{
    ...
    public OfferingPlaceholder(Guid id, Guid offeringId, string name, string? description, string position, string decorationMethod, int width, int height, IReadOnlyList<Guid> variantIds, bool isArchived, DateTimeOffset createdAt, DateTimeOffset updatedAt, string metadataJson = "{}")
    {
    ...
    public bool Equals(OfferingPlaceholder? other) => other is not null
        ...
        && MetadataJson == other.MetadataJson && _variantIds.SequenceEqual(other._variantIds);

    public override int GetHashCode()
    {
        var hash = new HashCode();
        hash.Add(Id); hash.Add(OfferingId); hash.Add(Name); hash.Add(Description); hash.Add(Position); hash.Add(DecorationMethod); hash.Add(Width); hash.Add(Height); hash.Add(IsArchived); hash.Add(CreatedAt); hash.Add(UpdatedAt); hash.Add(MetadataJson);
