...
public sealed class SqliteSnowcloneRepositoryTests
{
    ...
    [Fact]
    public async Task LoadAsync_FreshDatabaseCreatesVersionSixEmptyLibrary()
    {
        ...
        Assert.Equal(6, await ReadUserVersionAsync(path));
    }
    ...
    [Fact]
    public async Task LoadAsync_MigratesPopulatedVersionFiveWithoutChangingWorkspace()
    {
        using var tempDirectory = new TemporaryDirectory();
    ...
    public async Task Repositories_CanOpenInEitherOrder()
    {
        ...
        Assert.Equal(6, await ReadUserVersionAsync(firstPath));
        Assert.Equal(6, await ReadUserVersionAsync(secondPath));
    }
