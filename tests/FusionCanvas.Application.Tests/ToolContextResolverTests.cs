...
public class ToolContextResolverTests
{
    ...
    public void Resolve_ItemContextIncludesSelectedItemParentPathTagsAndAvailability()
    {
        ...
        Assert.Equal(sample.Item.Id, resolution.Context?.SelectedItem?.Id);
        ...
        Tag Tag)
    {
        public static ToolContextSample Create()
        {
            ...
            var listing = new Item(Guid.NewGuid(), store.Id, niche.Id, group.Id, "Pumpkin espresso", null, ItemStatus.Draft, WorkflowStage.Idea, false, now, now, """{"phrase":"Pumpkin power"}""");
            var otherListing = new Item(Guid.NewGuid(), store.Id, niche.Id, group.Id, "Latte leaves", null, ItemStatus.Draft, WorkflowStage.Listing, false, now, now, "{}");
            var archivedListing = new Item(Guid.NewGuid(), store.Id, niche.Id, group.Id, "Retired roast", null, ItemStatus.Draft, WorkflowStage.Idea, true, now, now, "{}");
