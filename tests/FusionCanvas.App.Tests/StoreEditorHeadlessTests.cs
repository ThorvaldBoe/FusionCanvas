...
public class StoreEditorHeadlessTests
{
    ...
    public void MockupPreview_WithoutImageShowsCompactUnavailableStateAndNoRectangle()
    {
        ...
        var guidance = string.Join(" ", window.GetVisualDescendants().OfType<TextBlock>().Where(IsEffectivelyVisible).Select(block => block.Text));
    ...
    public void ProviderImageSelection_RendersAvailableEmptyUnavailableAndErrorGuidance()
    {
        ...
        foreach (var testCase in cases)
        {
            ...
            var selector = window.GetVisualDescendants().OfType<ComboBox>()
                .Single(combo => IsEffectivelyVisible(combo) && AutomationProperties.GetName(combo) == "Provider mockup image");
            Assert.NotNull(selector);
            var state = window.GetVisualDescendants().OfType<TextBlock>()
            ...
            var recovery = window.GetVisualDescendants().OfType<TextBlock>()
