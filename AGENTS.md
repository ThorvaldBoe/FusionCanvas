...
- For each new or materially changed user-facing feature, explicitly decide whether real-desktop Appium coverage is warranted. When it is eligible, record the expected journey count, scenario-pack placement, shared starting state, user-action steps, visible and durable outcomes, cadence/filter, and lower-layer coverage. When it is not warranted—or a small maintenance change does not justify a new journey—record the rationale rather than adding ceremonial coverage.
...
- An optional, project-scoped OpenCode agent loop lives in `.opencode/agents/`: the primary `fc-coordinator` orchestrates `fc-spec-writer` (explore/propose), `fc-reviewer` (single reviewer for explore, proposal, specs, and code), read-only consultants (`fc-architect`, `fc-ui-specialist`, `fc-business-analyst`), `fc-implementer`, and `fc-verifier` (final verification only). It is an alternative driver for the same OpenSpec lifecycle, artifacts, and gates described above — not a separate process — and is documented in `.opencode/README.md`.
...
- Treat Appium tests as a supplemental real-desktop lane. Group ordered actions that intentionally share an isolated application state into a named scenario pack; use one fresh disposable workspace/database per pack, not one reset for every step. Traits select and describe packs, while xUnit collections control fixture/parallelization concerns and must not be treated as an ordering mechanism. Do not make unrelated test methods depend on runner execution order.
...
- For every implementation or bug-fix request, once the requested change has been implemented and proportionately verified, the default completion workflow is to commit the intended changes, push the branch, create a pull request against `main`, and merge it when the pull request is mergeable and repository requirements are satisfied. This applies to small maintenance fixes even when no OpenSpec change is required. Do not stop to ask whether these delivery steps should be performed. Skip them only when the user explicitly asks to hold, test or taste-test, review, avoid merging, or use another completion workflow; leave the pull request open and report when verification, mergeability, required checks, or approval is genuinely blocked. The user's request to fix or implement a change implies authorization for this normal scoped delivery workflow; ask separately only when an external system requires an action-time confirmation or the requested scope is materially ambiguous.
